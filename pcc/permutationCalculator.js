    ...
    while (permutationsString.length > 0) {
      ...
      if (currentlyInPermutation) {
        ...
        if (char === "(") {
          ...
          permutationsString.startsWith("Id ") ||
          permutationsString == "Id"
        ) {
    ...
    permutation.split("(").forEach((clause) => {
      if (clause.length > 3) {
        ...
        for (let i = 0; i < clauseNumbers.length; i++) {
          ret[clauseNumbers[i] - 1] =
            clauseNumbers[(i + 1) % clauseNumbers.length] - 1;
        }
      }
    });
  ...
  static composePermutation(g, f) {
    ...
    for (const row in fArray) {
      ...
      for (const column in gArray) {
        const composed = PermutationCalculator.#compose(
          gArray[column],
          fArray[row],
          highestNumber
        );
        thisResultRow[gArray[column]] =
          PermutationCalculator.#toHumanVisualisation(composed);
      }
